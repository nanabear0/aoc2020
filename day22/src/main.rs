...
fn main() {
    ...
    println!(
        "part1: {:?}",
        plej(&mut p1.clone(), &mut p2.clone(), false).1
    );
    println!("part2: {:?}", plej(&mut p1, &mut p2, true).1);
}

fn plej(p1: &mut VecDeque<usize>, p2: &mut VecDeque<usize>, recursive: bool) -> (usize, usize) {
    ...
    while !p1.is_empty() && !p2.is_empty() {
        ...
        let winner;
        if recursive && p1.len() >= c1 && p2.len() >= c2 {
